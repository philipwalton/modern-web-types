...
//
// For each scope with a delta it writes test/generated/<scope>.replace.ts,
// which references pkg/lib.<lib>.d.ts and asserts every delta symbol resolves
// there. Each assertion is type-only, so there is nothing to run and no unused
// value concern:
//   - new interface I / alias A       →  type _ = I;           (reachable as a type)
//   - new global var V / function F   →  type _ = typeof V;    (reachable as a value)
//   - member M added to interface P   →  type _ = P["M"];      (P carries M)
//   - namespace member P.M added      →  type _ = typeof P.M;
// A generic interface (e.g. XRHandIterator) can't be named without type args,
// so its type-reachability assertion is skipped — check-libs.ts typechecks each
// lib whole, so nothing goes unchecked.
//
...
function assertions(scope: Scope): string[] {
  ...
  // One assertion per distinct alias name.
...
for (const scope of deltaScopes) {
  ...
  write(
    ...
    [`../../pkg/${libFile(scope)}`],
    body,
    `Asserts every ${scope.name} delta symbol resolves in the complete ` +
      `one-engine ${scope.lib} lib.`,
  );

  console.log(`[${scope.name}] wrote ${scope.name}.replace.ts (${body.length} assertions)`);
}
